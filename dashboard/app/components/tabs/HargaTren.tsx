...
// Forecasts and daily prices exist for the 8 IHK cities (PIHPS panel).
...
export default function HargaTren({
  ...
  const cities = useMemo(() => kabupaten.filter((k) => k.tier === "TIER_1_HIGH"), [kabupaten]);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-emerald-100/80 mt-0.5">Harga harian PIHPS 8 kota IHK, prakiraan 30 hari dengan pita 80% terkalibrasi, dan anomali dari scanner Hampel/MAD.</p>
        </div>
      ...
      <p className="text-[11px] text-emerald-100/80">
        Scan anomali {fmtDate(meta?.data_as_of.anomaly_scan_generated_at)} · prakiraan dibuat {fmtDate(meta?.data_as_of.forecast_generated_at)} · metode interval: {meta?.data_as_of.forecast_interval_methods.join(", ") ?? "..."}. Kabupaten Tier 2 (non-IHK) belum punya harga harian; ini batas data publik, bukan engine.
      </p>
    </div>
  );
}
